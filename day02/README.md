...
- Bedrockのモデル呼び出し方法（boto3等）
    boto3を利用することで、PythonからAWSの各種サービスを操作できる
    今回のモデル呼び出しにはbedrock-runtimeを使用

- 利用する認証方式（研修の指示に従う）
    trainingプロファイルを作成し、AWS_PROFILEで使用するプロファイルを指定
    認証情報が正しく設定されていない場合は、Bedrockを呼び出す前に認証エラーになる

- タイムアウト/リトライの考え方
    タイムアウトは、AWSからの応答を無制限に待ち続けないために設定
    リトライは、一時的な通信エラーなどに対して再度リクエストする仕組み。
    ただし、認証エラーやパラメータ不正など、再実行しても解決しないエラーにはリトライしない
